...
public enum Quiet {
    ...
    ///
    /// Сорок две секунды. Восемь оказалось мало: claude закрывает ответ и когда
    /// ждёт не человека, а свою же фоновую задачу, и такие паузы вызывали зов
    /// на середине работы. Отличить эти два случая изнутри нельзя, в транскрипте
    /// оба выглядят как законченный ответ, поэтому лечим выдержкой.
    ///
    /// Долгие паузы всё равно позовут, но такой зов сам гаснет, едва работа
    /// продолжится: за этим следит проверка транскрипта в приложении.
    public static let delay: TimeInterval = 42
